...
FIRST_PATH = True
if  len(sys.argv) > 1 and sys.argv[1] in ["second"]:
    FIRST_PATH = False
BATCH_SIZE = 4
...
def getLogdir(training_mode):
    return './logs/catalyst_segmentation4c'
...
if __name__ == "__main__":
    ...
    df_pl = pd.read_csv("submission_segmentation.csv")
...
in_channels=4)
    ...
    if not TRAINING:
        ...
        if FIRST_PATH:
            MASK_DIR = "decoded_ss_rotate"
        else:
            MASK_DIR = "decoded_realsub_aligned"
        test_ds = tu.ITSTestDataset4C(test_files, test_mask_dir=MASK_DIR)
    ...
    for i, (train_inds, test_inds) in enumerate(kf.split(image_files, image_labels)):
        ...
        train_ds = tu.ITSDataset4c(train_files, train_transforms=[albu.HorizontalFlip(), albu.VerticalFlip(), albu.ShiftScaleRotate()], blur_mask=False)
        print(train_ds[0], np.count_nonzero(train_ds[0]["image"][ 3, :, :]))
        val_ds = tu.ITSDataset4c(valid_files)
    ...
    if not TRAINING:
        ...
        output.to_csv("submission_segmentation_4channel.csv", index=False)
